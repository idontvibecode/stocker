...
export default function NavBar({ activeTab, onTabChange }) {
  return (
    <>
      {/* Desktop */}
      <nav className="hidden sm:block" style={{ backgroundColor: '#1A2E23' }}>
        <div className="max-w-2xl mx-auto px-6 flex items-center justify-between h-14">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-display text-white text-base tracking-wide">Stocker</span>
          </div>
      ...
      {/* Mobile Top */}
      <nav className="sm:hidden" style={{ backgroundColor: '#1A2E23' }}>
        <div className="px-4 flex items-center h-12">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md flex items-center justify-center shrink-0" style={{ backgroundColor: '#D97706' }}>
            ...
            <span className="font-display text-white text-sm tracking-wide">Stocker</span>
          </div>
        </div>
      </nav>

      {/* Mobile Bottom */}
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-40 border-t" style={{ backgroundColor: '#1A2E23', borderColor: 'rgba(255,255,255,0.08)' }}>
        <div className="flex pb-safe">
          {tabs.map(({ id, label, Icon }) => (
            ...
            >
              <div className={`p-1.5 rounded-lg transition-all ${activeTab === id ? 'bg-white/10' : ''}`}
                style={{ color: activeTab === id ? '#D97706' : 'rgba(255,255,255,0.3)' }}>
                <Icon active={activeTab === id} />
              </div>
