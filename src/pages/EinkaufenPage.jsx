    ...
    <div className="flex flex-col gap-4">
      ...
      {planLeer ? (
        <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
          ...
          <p className="font-semibold text-zinc-900 text-base mb-1">Kein Wochenplan</p>
          <p className="text-zinc-400 text-sm mb-5">Zuerst Schritt 2 ausfüllen.</p>
          <button
            onClick={() => navigateTo('planen')}
            className="w-full py-3 bg-amber-500 text-white rounded-xl font-semibold text-sm cursor-pointer active:bg-amber-600 transition-all"
          >
            Zum Wochenplan
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
          ...
          <p className="font-semibold text-zinc-900 text-base mb-1">Alles vorhanden!</p>
          <p className="text-zinc-400 text-sm">Du hast alle Zutaten für diese Woche.</p>
        </div>
        ...
        <>
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 pt-4 pb-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <p className="text-xs font-medium text-zinc-400">Einkaufsliste</p>
                <span className="bg-zinc-100 text-zinc-500 text-[11px] px-1.5 py-0.5 rounded-full font-medium">{liste.length}</span>
              </div>
              {(() => {
                ...
                return vormarkierteNamen.length > 0 && (
                  <button
                    ...
                    className="text-xs font-medium text-amber-600 cursor-pointer active:text-amber-800"
                  >
            ...
            <div className="flex flex-col">
              {fehlend.map((z, i) => {
                ...
                return (
                  <div key={z.name}>
                    {i > 0 && <div className={`h-px mx-4 ${removed ? 'bg-zinc-50' : 'bg-zinc-100'}`} />}
                    <div className={`flex items-center gap-3 px-4 py-3 transition-colors ${removed ? 'bg-zinc-50' : ''}`}>
                      <span className={`flex-1 text-sm capitalize transition-all ${
                        removed ? 'line-through text-zinc-300' : 'text-zinc-800 font-medium'
                      }`}>
                        {z.name}
                      </span>
                      {!removed && mengeLabel && (
                      ...
                      {removed && (
                        <span className="text-[10px] text-zinc-300 shrink-0">nicht kaufen</span>
                      )}
                      <button
                        ...
                        className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all cursor-pointer ${
                          removed
                            ...
                            : 'bg-zinc-100 text-zinc-400 active:bg-rose-50 active:text-rose-400'
                        }`}
                      >
                        {removed
          ...
          <button
            onClick={teilen}
            className={`w-full py-4 rounded-2xl font-semibold text-base tracking-wide active:scale-[0.97] transition-all cursor-pointer flex items-center justify-center gap-2.5 ${
              geteilt
          ...
          <p className="text-center text-zinc-400 text-xs -mt-2">{liste.length} Zutaten</p>
        </>
      )}
    </div>
  )
